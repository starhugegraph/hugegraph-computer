...
public class HGModularityOptimizer {
    ...
    public HGModularityOptimizer(Config config) {
        this.config = config;
        this.maxId = -1;
        this.initialCapacity = 50000000;
        //config.get(ComputerOptions.INITIAL_CAPACITY);
        this.idMap = HashBiMap.create(this.initialCapacity);
        this.weightKey = "";//config.get(ComputerOptions.WEIGHT_PROPERTY_KEY);
        this.delimiter = " ";//config.get(ComputerOptions.FILE_DELIMITER);
    }

    public void runAlgorithm() throws IOException {
        int algorithm = ALGORITHM;
        int modularityFunction = 1;
                //this.config.get(ComputerOptions.MODULARITY_FUNCTION);
        double resolution = 1.0;//this.config.get(ComputerOptions.RESOLUTION);
        int nRandomStarts = 10;//this.config.get(ComputerOptions.RANDOM_STARTS);
        int nIterations = 100;//this.config.get(ComputerOptions.ITERATIONS);
        long randomSeed = 100;//this.config.get(ComputerOptions.RANDOM_SEED);
